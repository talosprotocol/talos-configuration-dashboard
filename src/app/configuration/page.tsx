...
export default function ConfigurationPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      // 1. Check contracts version
      const ver = await adapter.getContractsVersion();
      setContractsVersion(ver.contracts_version);
      
      // Strict Version Mismatch Block
      if (ver.contracts_version !== "unknown" && ver.contracts_version !== REQUIRED_VERSION) {
          setVersionMismatch(true);
          setError(`Contracts Version Mismatch: Backend has ${ver.contracts_version}, Dashboard requires ${REQUIRED_VERSION}`);
          return; // Block loading further data
      }
